...
mod = SourceModule("""
__global__ void calculate_eta(int *warp_results, float *eta) 
{
    int idx = threadIdx.x;
...
__global__ void calculate_execution_efficiency(
            int total_instructions, int divergent_branches,
            float *efficiency) 
{
    efficiency[0] = (total_instructions - divergent_branches) /
                    (float)total_instructions;
}
__global__ void manage_latency
           (float load, float overhead, float eta, float *latency)
{
           latency[0] = (load + overhead ) / eta;
}      
__global__ void optimize_tasks(int *tasks_arr, int n)
{
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if(idx < n)
    {
       tasks_arr[idx] = tasks_arr[idx] * 2;
    }
}    
...
optimized_task_arr = np.empty_like(task_arr)
cuda.memcpy_dtoh(optimized_task_arr, task_arr_gpu)
print("Optimized Tasks:",optimized_task_arr)
...
calculate_execution_efficiency = mod.get_function("calculate_execution_efficiency")
calculate_execution_efficiency(np.int32(100), np.int32(10), cuda.Out(execution_efficiency), block=(1, 1, 1))
