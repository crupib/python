...
def separator_change(
    in_file, out_files, separator, overwrite=False, check_for_collision=False
):
    ...
    if not overwrite:
        for file in out_files:
            if Path(file).exists():
                raise ValueError(
                    "overwrite was not specified and out-file already exists."
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("in_file", help="The input file to change the separator in")
    # need at least one output file, thus nargs="+" is used
    parser.add_argument("out_files", nargs="+", help="The output file(s) to write to")
