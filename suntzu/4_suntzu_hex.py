...
def draw_wrapped_text(text, x, y, width_chars, color=TEXT, f=font, line_height=21, max_lines=None, center=False, center_x=None):
...
def load_unit_images_for_scenario(scenario_key):
    ...
    for role, filename in role_to_file.items():
        if scenario_key == "normaninv":
            paths = [
                ASSET_ROOT / "normaninv" / filename,
                UNIT_ASSET_ROOT / "common" / filename,
            ]
        else:
            paths = [
                UNIT_ASSET_ROOT / scenario_key / filename,
                UNIT_ASSET_ROOT / "common" / filename,
            ]
