...
def move_unit(unit, target):
    ...
    if terrain == "river" and unit.role != "naval":
        message = "Use a bridge to cross river hexes."
        return
...
def ai_turn():
    ...
    for e in enemies:
        ...
        if current_scenario.key == "gaugamela" and e.role == "elephant":
            ...
            beach_targets = [p for p in targets if tiles[p.pos].terrain in ["beach", "supply", "bridge"]]
        ...
        if e.role == "chariot":
            ...
            candidates.sort(key=lambda c: (hex_distance(c, target.pos), 0 if tiles[c].terrain in ["plain", "desert", "road"] else 1))
            ...
            candidates.sort(key=lambda c: (10 if tiles[c].terrain in ["bocage", "forest"] else 0, hex_distance(c, target.pos)))
        else:
            candidates.sort(key=lambda c: hex_distance(c, target.pos))
