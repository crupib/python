...
def go_to_scenario_select():
    global screen_mode, selected, current_scenario, turn, phase, message
    global assessment_points, deployment_points, player_morale, enemy_morale, logistics, turn_number
...
def load_unit_images_for_scenario(scenario_key):
    ...
    roles = [
        "commander", "cavalry", "infantry", "elite infantry", "phalanx",
        "skirmisher", "guard", "artillery", "chariot", "elephant",
        "paratrooper", "armor", "naval", "bunker", "aircraft",
        "mechanized", "airdefense",
    ]
...
def valid_deploy_preview(q, r):
    ...
    if current_scenario.key == "sixdaywar":
        return q <= 4
...
def attack_unit(attacker, defender):
    ...
    if defender_tile.terrain in ["rough", "heights", "village", "bocage", "bunker", "airfield", "objective"]:
        terrain_bonus += 1
...
def estimate_ai_intent():
    ...
    elif current_scenario.key == "sixdaywar":
...
def special_feint():
    global assessment_points, deployment_points, message, enemy_morale, logistics
    ...
    if current_scenario.key == "gaugamela":
        ...
        logistics = min(logistics + 5, 140)
        message = "Preemptive air operation executed. Enemy air and command assets are disrupted."
...
def ai_turn():
    ...
    for e in enemies:
        ...
        if e.role == "chariot":
            ...
            candidates.sort(key=lambda c: (hex_distance(c, target.pos), 0 if tiles[c].terrain in ["plain", "desert"] else 1))
            ...
            candidates.sort(key=lambda c: (10 if tiles[c].terrain == "bocage" else 0, hex_distance(c, target.pos)))
        else:
            candidates.sort(key=lambda c: hex_distance(c, target.pos))
...
def check_victory():
    ...
    if current_scenario.key == "sixdaywar" and turn_number > 8:
        return f"{current_scenario.enemy_side} wins. The war dragged into attrition."
...
def setup_sixdaywar():
    ...
    turn = "Israel"
    phase = "DEPLOY"
    message = "Six-Day War deployment. Use speed, air superiority, and economy of force to avoid attrition."
    assessment_points = 3
    deployment_points = 7
    player_morale = 100
    enemy_morale = 100
...
def scenarios_for_selected_principle():
    principle_key = principles[selected_principle_index].key
    return [s for s in scenarios if s.principle_key == principle_key]


def start_scenario(scenario):
    global current_scenario, screen_mode, dropdown_open
...
def draw_dropdown():
    ...
    selected_principle = principles[selected_principle_index]
    draw_text(
        f"Principle: {selected_principle.title}  v",
        0,
    ...
    if dropdown_open:
        ...
        for i, principle in enumerate(principles):
            rect = pygame.Rect(
                ...
                PRINCIPLE_DROPDOWN_RECT.y + PRINCIPLE_DROPDOWN_RECT.height + i * option_h,
                PRINCIPLE_DROPDOWN_RECT.width,
                option_h,
            )
            opt_hover = rect.collidepoint(mx, my)
            opt_color = BUTTON_HOVER if opt_hover else PANEL

            pygame.draw.rect(screen, opt_color, rect)
            ...
            draw_text(
                ...
                0,
                rect.y + 11,
                TEXT if i != selected_principle_index else SELECT,
                font,
                center=True,
...
def draw_scenario_select():
    ...
    center_x = WIDTH // 2
    draw_text("SUN TZU HEX TACTICS", 0, 35, TEXT, big_font, center=True, center_x=center_x)
    draw_text("Select a principle, then choose a scenario", 0, 72, MUTED, font, center=True, center_x=center_x)

    draw_dropdown()

    principle = principles[selected_principle_index]
    draw_wrapped_text(
    ...
    visible_scenarios = scenarios_for_selected_principle()
    ...
    for i, scenario in enumerate(visible_scenarios):
        ...
        draw_text(f"{i + 1}. {scenario.title}", 0, y + 18, TEXT, big_font, center=True, center_x=card_center)
        draw_text(scenario.subtitle, 0, y + 53, MUTED, small_font, center=True, center_x=card_center)
    ...
    draw_text("Press 1-9, click a scenario, or click the principle selector.", 0, 720, TEXT, font, center=True, center_x=center_x)
...
def draw_sidebar():
    ...
    y = 54
    principle = next(p for p in principles if p.key == current_scenario.principle_key)
    y = draw_wrapped_text(principle.title, x, y, 38, SELECT, small_font, 18, max_lines=2, center=True, center_x=center_x)
...
def draw_legend():
    ...
    draw_wrapped_text(
        "Terrain: green=plain, tan=prepared/desert, gray=rough/bunker/airfield, blue=supply/deploy/sea, brown=heights/objective, sand=beach, dark green=bocage",
        35,
        739,
        80,
        TEXT,
...
def handle_game_key(event):
    global selected, message, assessment_points, deployment_points
...
def handle_scenario_click(mx, my):
    global dropdown_open, selected_principle_index
...
def handle_scenario_key(event):
    ...
    if event.key == pygame.K_UP:
        selected_principle_index = (selected_principle_index - 1) % len(principles)
        dropdown_open = False
        return
    ...
    if pygame.K_1 <= event.key <= pygame.K_9:
        ...
        if idx < len(visible_scenarios):
            start_scenario(visible_scenarios[idx])
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if screen_mode == "SCENARIO_SELECT":
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if game_over and BACK_BUTTON_RECT.collidepoint(*pygame.mouse.get_pos()):
                    go_to_scenario_select()
                elif not game_over:
    ...
    if screen_mode == "SCENARIO_SELECT":
        ...
        if game_over:
            ...
            draw_text("Use Back to Scenarios, M, or ESC.", 0, 405, TEXT, font, center=True, center_x=590)
