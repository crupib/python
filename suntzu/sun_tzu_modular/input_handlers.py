...
import pygame
from constants import WIDTH, BACK_BUTTON_RECT, PRINCIPLE_DROPDOWN_RECT
from hex_utils import pixel_to_axial
from logic import start_scenario, reveal_around, estimate_ai_intent, special_feint, begin_battle, end_player_turn, deploy_unit, move_unit, attack_unit, get_unit_at
...
def handle_scenario_click(state, mx, my):
    ...
    if state.dropdown_open:
        option_h = 42
        for i in range(len(state.principles)):
            ...
            if rect.collidepoint(mx, my):
                state.selected_principle_index = i
