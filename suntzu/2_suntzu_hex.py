...
def load_unit_images():
    ...
    for role in roles:
        path = ASSET_DIR / role_to_file[role]
        if path.exists():
...
def reveal_around(pos, radius=2):
    ...
    message = f"Scouting revealed {revealed_count} enemy formation(s)." if revealed_count else "Scouting improved terrain knowledge."
...
def estimate_ai_intent():
    ...
    if current_scenario.key == "gaugamela":
        if any(u.role == "chariot" and u.hp > 0 for u in ai_units()):
            hints.append("Chariots favor prepared plain.")
...
def special_feint():
    ...
    if current_scenario.key == "gaugamela":
        ...
        message = "Feint right executed. Persian cavalry shifts outward."
        ...
        message = "False weakness shown. Coalition forces overextend."
...
def draw_units():
    for u in units:
        ...
        draw_text(str(u.hp), cx - 9, cy + 20, TEXT)
