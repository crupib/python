...
for q in range(MAP_COLS):
    for r in range(MAP_ROWS):
        ...
        # Persian prepared ground for chariots and large shock units.
        if 8 <= q <= 13 and 3 <= r <= 8:
            terrain = "prepared"

        # Rough ground weakens cavalry, chariots, and elephants.
        if q >= 12 and r <= 3:
            terrain = "rough"
...
def attack_unit(attacker, defender):
    ...
    # Chariots suffer against disciplined phalanx.
    if defender.role == "phalanx" and attacker.role == "chariot":
        terrain_bonus += 2

    # Elephants are terrifying against cavalry, but vulnerable to skirmishers.
    elephant_bonus = 0
    ...
    damage = max(1, attacker.atk + elephant_bonus + random.randint(-1, 2) - terrain_bonus)
...
def estimate_ai_intent():
    ...
    elephants = [u for u in units if u.side == "Persia" and u.role == "elephant" and u.hp > 0]
    ...
    if elephants:
        hints.append("Elephants are slow shock units. Avoid cavalry contact; screen them with Agrianians or phalanx.")
...
def ai_turn():
    ...
    for p in persians:
        ...
        if p.role == "elephant":
            # Elephants prefer cavalry, then nearby infantry.
            cavalry_targets = [m for m in targets if m.role == "cavalry"]
            if cavalry_targets:
                target = min(cavalry_targets, key=lambda m: hex_distance(p.pos, m.pos))
            else:
        ...
        elif p.role == "elephant":
            # Elephants avoid rough terrain and lumber toward the nearest valuable target.
            candidates.sort(key=lambda c: (
...
def draw_legend():
    draw_text("Terrain: green=plain, tan=prepared ground, gray=rough, blue=supply/deployment, brown=elephants", 40, 740)
