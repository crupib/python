...
def draw_wrapped_text(text, x, y, width_chars, color=TEXT, f=font, line_height=21, max_lines=None):
...
def load_unit_images_for_scenario(scenario_key):
    ...
    for role, filename in role_to_file.items():
        scenario_path = ASSET_ROOT / scenario_key / filename
        common_path = ASSET_ROOT / "common" / filename
        ...
        if scenario_path.exists():
            ...
            UNIT_IMAGES[role] = None
            continue

        try:
            img = pygame.image.load(str(path)).convert_alpha()
            img = pygame.transform.smoothscale(img, (UNIT_IMAGE_SIZE, UNIT_IMAGE_SIZE))
            UNIT_IMAGES[role] = img
        except pygame.error:
            UNIT_IMAGES[role] = None
...
def draw_fallback_unit_icon(u, cx, cy):
    ...
    draw_text(label, cx - 12, cy - 9, (255, 255, 255), small_font)
...
def draw_units():
    for u in units:
        ...
        draw_text(str(u.hp), cx - 8, cy + 21, TEXT, small_font)
...
def draw_sidebar():
    ...
    draw_text("SUN TZU HEX TACTICS", x, 18, TEXT, big_font)

    y = 54
    y = draw_wrapped_text(current_scenario.principle, x, y, 38, SELECT, small_font, 18, max_lines=2)
    y = draw_wrapped_text(current_scenario.title, x, y + 3, 38, TEXT, font, 21, max_lines=2)

    y += 10
    draw_text(f"Turn: {turn_number if turn_number else 'Pre-Battle'}", x, y)
    y += 23
    draw_text(f"Phase: {phase}", x, y)
    y += 30

    if phase == "DEPLOY":
        draw_text(f"Deployment Points: {deployment_points}", x, y)
        y += 25
    else:
        draw_text(f"Assessment Points: {assessment_points}", x, y)
        y += 22
        draw_text(f"{current_scenario.player_side} Morale: {player_morale}", x, y)
        y += 22
        draw_text(f"{current_scenario.enemy_side} Morale: {enemy_morale}", x, y)
        y += 22
        draw_text(f"Logistics: {logistics}", x, y)
        y += 25

    draw_text("Controls", x, y, TEXT, big_font)
    y += 38
    ...
    for item in controls:
        y = draw_wrapped_text(item, x, y, 39, TEXT, small_font, 18, max_lines=2)

    y += 12
    draw_text("Selected", x, y, TEXT, big_font)
    y += 38
    ...
    if selected:
        ...
        draw_text("None", x, y, TEXT, small_font)
        y += 22
    ...
    draw_text("Report", x, report_y, TEXT, font)
    draw_wrapped_text(message, x, report_y + 26, 41, TEXT, small_font, 18, max_lines=3)


def draw_legend():
    pygame.draw.rect(screen, (25, 24, 21), (25, 732, 1080, 42), border_radius=8)
    draw_wrapped_text(
        "Terrain: green=plain, tan=prepared, gray=rough, blue=supply/deploy, brown=heights, icy=frozen, red=village",
        35,
        742,
        115,
        TEXT,
...
while running:
    ...
    if screen_mode == "SCENARIO_SELECT":
        ...
        if game_over:
            ...
            draw_wrapped_text(game_over, 245, 335, 60, TEXT, big_font, 32, max_lines=2)
            draw_text("Press M to return to scenario selection.", 245, 405)
